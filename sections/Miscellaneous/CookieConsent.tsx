...
function CookieConsent({ title = "Cookies", text = "Guardamos estatísticas de visitas para melhorar sua experiência de navegação.", policy = {
    ...
    const id = useId();
    return (<>
      <div id={id} class={clx("transform-gpu translate-y-[200%] transition fixed bottom-0 w-screen z-50 sm:flex", "sm:bottom-2 sm:justify-cente")}>
        <div class={clx("p-4 mx-4 my-2 flex flex-col gap-4 shadow bg-base-100 rounded border border-base-200", "sm:flex-row sm:items-end")}>
