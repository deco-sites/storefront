...
function CampaignTimer({ expiresAt = `${new Date()}`, labels = {
    days: "days",
    hours: "hours",
    minutes: "minutes",
    seconds: "seconds",
    ...
    const TimeComponent: preact.FunctionalComponent<TimeComponentProps> = ({ id, label, time }) => (<div class="flex flex-col items-center">
      <span class="countdown font-normal text-xl lg:text-2xl">
        <span class="md:text-8xl text-6xl font-thin text-base-content tracking-[-3px]" id={`${id}::${time}`}/>
      </span>
    ...
    return (<>
      <div>
        <div class="container mx-auto flex flex-col lg:flex-row lg:items-center lg:justify-center lg:gap-16 py-16 sm:px-10 gap-4">
          <div id={`${id}::expired`} class="hidden text-sm text-center lg:text-xl lg:text-left lg:max-w-lg" dangerouslySetInnerHTML={{ __html: text || "Expired!" }}>
          </div>
      ...
      <script type="module" dangerouslySetInnerHTML={{ __html: useScript(snippet, expiresAt, id) }}/>
    </>);
}
