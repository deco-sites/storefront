...
function Notice({ title, description }: {
    ...
    return (<div class="flex flex-col justify-center items-center sm:items-start gap-4">
...
function Newsletter({ empty = {
...
}, label = "Sign up", placeholder = "Enter your email address", status, }: SectionProps<typeof loader, typeof action>) {
    ...
    return (<Section.Container class="bg-base-200">
      <div class="p-14 grid grid-flow-row sm:grid-cols-2 gap-10 sm:gap-20 place-items-center">
        ...
        <form hx-target="closest section" hx-swap="outerHTML" hx-post={useComponent(import.meta.url)} class="flex flex-col sm:flex-row gap-4 w-full">
          <input name="email" class="input input-bordered flex-grow" type="text" placeholder={placeholder}/>
