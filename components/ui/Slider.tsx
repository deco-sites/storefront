...
function Dot({ index, ...props }: {
    ...
    return (<button {...props} data-dot={index} aria-label={`go to slider item ${index}`} class={clx("focus:outline-none group", props.class?.toString())}/>);
}
...
const onLoad = ({ rootId, scroll, interval, infinite }: Props) => {
    function init() {
        ...
        const observer = new IntersectionObserver((elements) => elements.forEach((e) => {
