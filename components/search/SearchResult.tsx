...
function NotFound() {
    return (<div class="w-full flex justify-center items-center py-10">
      <span>Not Found!</span>
    </div>);
}
...
function PageResult(props: SectionProps<typeof loader>) {
    ...
    return (<div class="grid grid-flow-row grid-cols-1 place-items-center">
      ...
      <div data-product-list class={clx("grid items-center", "grid-cols-2 gap-2", "sm:grid-cols-4 sm:gap-10", "w-full")}>
        {products?.map((product, index) => (<ProductCard key={`product-card-${product.productID}`} product={product} preload={index === 0} index={offset + index} class="h-full min-w-[160px] max-w-[300px]"/>))}
      </div>
...
function Result(props: SectionProps<typeof loader>) {
    ...
    const results = (<span class="text-sm font-normal">
      ...
      <div id={container} {...viewItemListEvent} class="w-full">
        {partial
            ...
            : (<div class="container flex flex-col gap-4 sm:gap-5 w-full py-4 sm:py-5 px-5 sm:px-0">
              ...
              {device === "mobile" && (<Drawer id={controls} aside={<div class="bg-base-100 flex flex-col h-full divide-y overflow-y-hidden">
              ...
              <div class="grid place-items-center grid-cols-1 sm:grid-cols-[250px_1fr]">
                {device === "desktop" && (<aside class="place-self-start flex flex-col gap-9">
                ...
                <div class="flex flex-col gap-9">
                  {device === "desktop" && (<div class="flex justify-between items-center">
                      {results}
                      <div>
                        {sortBy}
                      </div>
