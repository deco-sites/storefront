...
function SignIn({ variant }: {
    ...
    const id = useId();
    return (<div id={id}>
      <a class={clx("btn btn-sm font-thin btn-ghost no-animation", variant === "mobile" && "btn-square")} href="/login" aria-label="Login">
        <Icon id="account_circle"/>
        {variant === "desktop" && <span>Sign in</span>}
      </a>
      <a class={clx("hidden", "btn btn-sm font-thin btn-ghost no-animation", variant === "mobile" && "btn-square")} href="/account" aria-label="Account">
        <Icon id="account_circle"/>
