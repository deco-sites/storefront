...
export const action = async (_: unknown, req: Request, ctx: AppContext) => {
    ...
    const wishlist = await ctx.invoke("site/actions/wishlist/submit.ts", { productID, productGroupID });
    return { wishlist };
};
...
function WishlistProvider({ wishlist }: {
    ...
    return (<form id={WISHLIST_FORM_ID} hx-post={useComponent(import.meta.url)} hx-trigger="submit" hx-disabled-elt="button[data-wishlist-button]">
      <input type="hidden" name="product-id"/>
      <input type="hidden" name="product-group-id"/>
      <button hidden/>

      <script type="application/json" dangerouslySetInnerHTML={{ __html: JSON.stringify(wishlist) }}/>
