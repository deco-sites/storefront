...
import Carousel, { Props as CarouselProps } from "$store/Sections/Layout/Carousel.tsx";
...
export default function Section({ placeholderItems, items, slider }: Props) {
  ...
  return (
    <Carousel layout={{itemWidth: 200}} {...slider} children={allItems.map((item) => <Card {...item} />)} />
  );
}
